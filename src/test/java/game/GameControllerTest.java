...
class GameControllerTest {
    ...
    void moveUpIntoFloor() {
        ctrl.move(-1, 0); // (5,4) → (4,4) which is floor in row 4 " #    #"
        Board b = ctrl.getBoard();
    ...
    void moveRightIntoFloor() {
        ctrl.move(0, 1); // (5,4) → (5,5) which is floor in " #  @ #"
        Board b = ctrl.getBoard();
    ...
    void pushCrateToGoal() {
        ...
        ctrl.move(-1, 0); // (5,4)→(4,4)
        ctrl.move(-1, 0); // (4,4)→(3,4)
        ctrl.move(-1, 0); // (3,4)→(2,4)
        ctrl.move(-1, 0); // (2,4)→(1,4)
        ctrl.move(0, -1); // (1,4)→(1,3)
        ctrl.move(1, 0);  // push crate from (2,3) to (3,3)
        ...
        assertTrue(ctrl.isLevelComplete(), "Level should be complete");
    }
    ...
    void cannotPushCrateIntoWall() {
        ...
        ctrl.move(-1, 0); // (5,4)→(4,4)
        ctrl.move(-1, 0); // (4,4)→(3,4)
        ctrl.move(-1, 0); // (3,4)→(2,4) — player right of crate
        ctrl.move(0, -1); // push crate (2,3)→(2,2), player→(2,3)
        ctrl.move(0, -1); // push crate (2,2)→(2,1), player→(2,2)
        ctrl.move(0, -1); // try push crate (2,1)→(2,0) which is '#' → blocked
    ...
    void restartResetsCratePosition() {
        ...
        ctrl.move(-1, 0);
        ctrl.move(-1, 0);
        ctrl.move(-1, 0); // player at (2,4)
        ctrl.move(0, -1); // push crate from (2,3) to (2,2)
